...
// Uses FM synthesis-like decay or multiple harmonics for a lush sound
export function playAirportChime(audioCtx: AudioContext, masterVolume: number): Promise<void> {
  return new Promise((resolve) => {
    const destination = audioCtx.destination;

    // Harmonic frequencies for airport announcement: F4, A4, C5, F5
    const notes = [349.23, 440.00, 523.25, 698.46];
    const duration = 0.5; // duration of each individual note
    const spacing = 0.18; // interval spacing between notes

    // Create a compressor to glue the notes together
    ...
    masterGain.gain.setValueAtTime(masterVolume * 0.4, audioCtx.currentTime);
    ...
    notes.forEach((freq, index) => {
      ...
      // Sub-harmonic oscillator for a fuller, warmer analog feel (1 octave below, subtle volume)
      const subOsc = audioCtx.createOscillator();
      subOsc.type = "sine";
      subOsc.frequency.setValueAtTime(freq / 2, startTime);

      // Gain node for decay envelope
      const gainNode = audioCtx.createGain();
      gainNode.gain.setValueAtTime(0, startTime);
      gainNode.gain.linearRampToValueAtTime(0.8, startTime + 0.04); // attack
      gainNode.gain.exponentialRampToValueAtTime(0.001, startTime + duration); // decay

      // Gain node for sub-oscillator (lower volume)
      const subGainNode = audioCtx.createGain();
      subGainNode.gain.setValueAtTime(0, startTime);
      subGainNode.gain.linearRampToValueAtTime(0.25, startTime + 0.04);
      subGainNode.gain.exponentialRampToValueAtTime(0.001, startTime + duration);
...
// Play closing chime: descending chord
export function playClosingChime(audioCtx: AudioContext, masterVolume: number): Promise<void> {
  return new Promise((resolve) => {
    const destination = audioCtx.destination;
    const notes = [523.25, 440.00, 349.23]; // C5, A4, F4 (descending)
    const duration = 0.6;
    const spacing = 0.2;

    const masterGain = audioCtx.createGain();
    masterGain.gain.setValueAtTime(masterVolume * 0.3, audioCtx.currentTime);
